...
> **Обязательное условие для работы с сервером:** на Mac должен быть приватный SSH-ключ из локального `ops/vps-ssh.json`. Без него вход на `92.113.147.193` невозможен. Этот файл намеренно отсутствует в GitHub и передаётся на Mac отдельно.
...
Для входа используется пара SSH-ключей:

- приватный ключ хранится в поле `privateKey` локального JSON и обязателен на Mac;
- соответствующий публичный ключ уже добавлен на сервер в `~/.ssh/authorized_keys`;
- публичный ключ не заменяет приватный и сам по себе не позволяет войти;
- пароль, приватный ключ и полный JSON нельзя добавлять в GitHub или логи Codex.

На Windows доступ хранится отдельно в:
...
Из JSON нужно извлечь поле `privateKey` в локальный файл, не печатая содержимое ключа в терминал, чат или логи. Файл должен начинаться строкой `-----BEGIN RSA PRIVATE KEY-----`. После этого:
...
Проверить, что приватный ключ соответствует установленному публичному ключу:

```bash
ssh-keygen -y -f ~/.ssh/vpnboss_denmark > ~/.ssh/vpnboss_denmark.pub
ssh-keygen -lf ~/.ssh/vpnboss_denmark.pub
ssh -o IdentitiesOnly=yes -i ~/.ssh/vpnboss_denmark ElValleChat@92.113.147.193 'echo SSH_OK'
```

Ожидаемый результат последней команды: `SSH_OK`. Если получено `Permission denied (publickey)`, нельзя переходить к серверным изменениям: сначала нужно правильно перенести приватный ключ.
